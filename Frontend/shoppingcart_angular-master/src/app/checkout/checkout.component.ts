  ...
  checkoutCart(){
    ...
    if(this.pay_type == "cash_on_delivery"){
      ...
      let request = {
        ...
        "amount":this.amount,
        "cvcNumber":this.cvc_number,
        "cardholderName":this.card_holder_name

    }
    if(this.credit_card_number == ""){
      alert("please add your credit card number");
    ...
    if(this.amount == ""){
      alert("please add your amount");
          ...
          if(this.cvc_number == ""){
            alert("please add your cvc number");
          }
          if(this.card_holder_name == ""){
            alert("please add card holder's name");
          }
    this.http.postRequestWithToken("api/order/checkout_order",request).subscribe((data:any)=>{
      alert("checkout and your payment process completed.Your Order is processed..");
      this.cartService.getCartDetailsByUser();
      this.router.navigate(['']);
   },error=>{
    alert("Error while fetching the cart Details");
